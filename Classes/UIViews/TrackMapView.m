...
static UIImage * vscFlagImage = nil;
static UIImage * vscinFlagImage = nil;
...
- (void)dealloc
{
	...
	[vscFlagImage release];
	[vscinFlagImage release];
...
- (void)InitialiseImages
{
	if(!flag_images_initialised_)
	{
		...
		vscFlagImage = [[UIImage imageNamed:@"StateVSC.png"] retain];
		vscinFlagImage = [[UIImage imageNamed:@"StateVSCIn.png"] retain];
	}
	else
	{
...
- (void) drawTrack
{
	...
	{		
		...
		if(!midasStyle && !isZoomView && !smallSized)
		{
			...
			else if(track_state == TM_TRACK_VSC)
				[vscFlagImage drawAtPoint:flagPos];
			else if(track_state == TM_TRACK_VSCIN)
				[vscinFlagImage drawAtPoint:flagPos];
		}
	}
}
