...
@synthesize dataTimeout;
@synthesize requiresTransferSize;
...
- (void)OnReceiveWithSize: (CFSocketRef) socket_ref Data:(CFDataRef)data
{
	// Get size of data
...
- (void)OnReceiveNoSize: (CFSocketRef) socket_ref Data:(CFDataRef)data
{
	// Get size of data
	CFIndex data_size = CFDataGetLength(data);
	UInt8 * byte_ptr = (UInt8 *)CFDataGetBytePtr(data);
	...
	[transferData newTransfer:data_size];
	memcpy ( [[transferData getStream] inqBuffer], byte_ptr, data_size );
	[transferData handleCommand];
}
...
- (void)OnReceive: (CFSocketRef) socket_ref Data:(CFDataRef)data
{
	...
	if(requiresTransferSize)
		[self OnReceiveWithSize:socket_ref Data:data];
	else
		[self OnReceiveNoSize:socket_ref Data:data];
}
...
- (void) verifySocketTimer: (NSTimer *)theTimer
{
	...
	if ( timeNow - lastReceiveTime > dataTimeout )
		[self OnDisconnect:false];
}
