...
- (void)viewWillAppear:(BOOL)animated;    // Called when the view is about to made visible.
{
	...
	if ( [[RaceMapSponsor Instance] supportsLocation] )
	{
		[locationLabel setHidden:false];
		[locationSwitch setHidden:false];
		locationSwitch.selectedSegmentIndex = [RaceMapSponsor Instance].location;
	}
	else
	{
...
- (void) updateServerState
{
	...
	else
	{
		...
		if([[RaceMapCoordinator Instance] videoConnectionStatus] == BPC_CONNECTION_CONNECTING_)
...
{
	NSString *text = [sender text];
	[[RaceMapCoordinator Instance] SetVideoServerAddress:text];
}

-(IBAction)videoURLChanged:(id)sender
{
	NSString *text = [sender text];
	[[RaceMapCoordinator Instance] SetVideoServerURL:text];
}

-(IBAction)videoConnectPressed:(id)sender
{
	if ( [[RaceMapCoordinator Instance] videoConnectionStatus] == BPC_CONNECTION_SUCCEEDED_ )
	{
		[[BasePadMedia Instance] disconnectVideoServer];
	}
	else if ( [[RaceMapCoordinator Instance] videoConnectionStatus] != BPC_CONNECTION_CONNECTING_ )
	{
		[videoServerTwirl setHidden:false];
		[videoServerTwirl startAnimating];
		NSString *ip_text = [video_ip_address_edit_ text];
		[[RaceMapCoordinator Instance] SetVideoServerAddress:ip_text];
		NSString *url_text = [video_url_edit_ text];
		[[RaceMapCoordinator Instance] SetVideoServerURL:url_text];
...
- (IBAction)loadPressed:(id)sender
{
	...
	if ( eventName != nil && [eventName length] > 0
	  && sessionName != nil && [sessionName length] > 0 )
	{
		[[RaceMapCoordinator Instance] loadSession:eventName Session:sessionName];
	}
}
...
{
	[[RaceMapSponsor Instance] setLocation:locationSwitch.selectedSegmentIndex];
}
